...
class _BillingPageState extends State<BillingPage> {
  ...
  Future<void> _initializeApp() async {
    try {
      ...
      // Fetch billing data
      await _fetchBillings(); // Will also assign _filteredbillings

      // Fetch billing map (house-wise dues)
      final map = await fetchLatestOutstandingDuesPerHouse();
      setState(() {
        houseChargesMap = map;
      });

      // Fetch additional billing details if needed
  ...
  Future<void> _fetchBillings() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _billings = decoded;
          _billingDetails = _billings;
          _filteredbillings = _billings;
          _isLoading = false;
        });
      } else {
  ...
  void _onSearchChanged() {
    ...
    setState(() {
      if (query.isEmpty) {
        _filteredbillings = _billings; // Reset to original list if search is cleared
      } else {
        _filteredbillings = _billings.where((billing) {
          final fullName = "${billing['first_name']} ${billing['last_name']}".toLowerCase();
          final hno = billing['house']['hno'].toString().toLowerCase();
          return fullName.contains(query) || hno.contains(query);
        }).toList();
      }
    });
  }
